...
const voteResult = (from, data) => {
  ...
  if (room.player.length === 0 || room.player[0].vote === 0) return 0;
  if (room.player.length > 1 && room.player[0].vote === room.player[1].vote) return 1;
...
async function pagi(conn, x, data) {
  ...
  return await conn.sendMessage(x.room, { image: { url: thumb1 }, caption: pagii(x), mentions: ment });
}
...
async function voting(conn, x, data) {
  ...
  return await conn.sendMessage(x.room, { image: { url: thumb2 }, caption: textnya, mentions: ment });
}
...
async function malam(conn, x, data) {
  ...
  if (hasil_vote === 0) {
    ...
    return conn.sendMessage(x.room, { image: { url: thumb3 }, caption: textnya }).then(() => {
      changeDay(x.room, data);
      voteDone(x.room, data);
      resetVote(x.room, data);
      clearAllVote(x.room, data);
      if (getWinner(x.room, data).status != null) return win(sesi(x.room, data), 1, conn, data);
    });
    ...
    if (hasil_vote.role === 'werewolf') {
      ...
      return await conn.sendMessage(x.room, { image: { url: thumb4 }, caption: textnya, mentions: ment }).then(() => {
        changeDay(x.room, data);
        voteDone(x.room, data);
        resetVote(x.room, data);
        clearAllVote(x.room, data);
...
async function win(x, t, conn, data) {
  ...
  if (getWinner(x.room, data).status === false || x.iswin === false) {
    ...
    return await conn.sendMessage(x.room, { image: { url: thumb5 }, caption: textnya, mentions: ment }).then(() => {
      delete data[x.room];
    });
