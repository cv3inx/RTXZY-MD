...
import { imageToWebp, videoToWebp, writeExifImg, writeExifVid } from '../lib/media/exif.js';

import { toAudio, toPTT, toVideo } from './media/converter.js';
...
import { proto, isGroupJid, isLidJid, isUserJid, isNewsletterJid, toUserJid, normalizeRecipientJid, splitJid, WA_BROWSERS, WA_DISCONNECT_REASONS } from 'zapo-js';
...
const createLogger = (existing = {}) => ({
  ...existing,
  info(...args) {
    console.log(chalk.bold.rgb(57, 183, 16)(`INFO [${shortDate()}]:`), chalk.cyan(util.format(...args)));
  },
  warn(...args) {
    console.log(chalk.bold.rgb(239, 225, 3)(`WARNING [${shortDate()}]:`), chalk.keyword('orange')(util.format(...args)));
  },
  error(...args) {
    console.log(chalk.bold.rgb(247, 38, 33)(`ERROR [${shortDate()}]:`), chalk.rgb(255, 38, 0)(util.format(...args)));
  },
  success(...args) {
    console.log(chalk.bold.rgb(0, 200, 120)(`SUCCESS [${shortDate()}]:`), chalk.green(util.format(...args)));
  }
});
...
if (WMI) {
  ...
  if (typeof WMI.prototype?.toJSON !== 'function' && WMI.prototype)
    WMI.prototype.toJSON = function toJSON() {
      return toPlainWebMessageInfo(this);
    };
}
if (MSG) {
...
export function areJidsSameUser(jid1, jid2) {
  const n1 = String(jid1 || '')
    .split('@')[0]
    .split(':')[0];
  const n2 = String(jid2 || '')
    .split('@')[0]
    .split(':')[0];
  return !!n1 && !!n2 && n1 === n2;
}

// @lid jids aren't mention/message-able directly; resolve to the real @s.whatsapp.net jid when possible.
export async function resolveLidJid(conn, jid) {
  jid = String(jid || '');
  if (!jid.endsWith('@lid')) return jid;
  const sync = conn.getJid(jid);
  if (sync && !String(sync).endsWith('@lid')) return sync;
  const async_ = await conn.getJidAsync(jid).catch(() => null);
  if (async_ && !String(async_).endsWith('@lid')) return async_;
  return jid;
}

/**
 * Nomor untuk ditampilkan ke manusia: format internasional kalau jid-nya
 * memang nomor telepon, digit apa adanya kalau bukan.
 *
 * Hanya jid @s.whatsapp.net yang boleh diformat. Digit LID (berapa pun
 * panjangnya) bukan nomor telepon — `224739230494840@lid` akan jadi
 * '+224 739230494840' yang palsu dan menyesatkan.
 */
export function displayJidNumber(jid) {
  if (!jid) return null;
  const raw = String(jid);
  if (!raw.endsWith('@s.whatsapp.net')) return raw.split('@')[0];
  const digits = raw.replace(/:\d+@/g, '@').split('@')[0];
  // 0@s.whatsapp.net = nomor tersembunyi/terverifikasi, jangan jadi '+0'
  if (!digits || digits === '0') return digits || null;
  return PhoneNumber('+' + digits).getNumber('international') || digits;
}

// Membangun regex prefix '^[...]' satu karakter dari array atau string
// karakter prefix (masing-masing di-escape, aman untuk char class).
export function buildPrefixRegex(chars) {
  const list = Array.isArray(chars) ? chars : String(chars).split('');
  return new RegExp('^[' + list.map((c) => c.replace(/[.*+?^${}()|[\]\\-]/g, '\\$&')).join('') + ']');
}
...
exports.attach = (conn) => {
  ...
  const seedOwnLid = () => {
    try {
      ...
      // Best-effort: sebelum pairing selesai kredensial belum ada. Dipanggil
      // ulang tiap 'connection.update', jadi cukup dilewati.
    }
  };
  seedOwnLid();
  conn.ev.on('connection.update', (update) => {
    ...
    if (update?.connection === 'open') {
      ...
      setTimeout(() => {
        ...
        conn.logger?.info?.('✅ Grace period complete, processing new messages');
      }, 10_000);
    }
  });
  ...
  conn.groupMetadata = async (jid) => {
    ...
    return {
      ...
      participants: (g.participants || []).map((p) => {
        ...
        const id = raw ? (String(raw).endsWith('@') ? String(raw) : String(raw).replace(/[^0-9]/g, '') + '@s.whatsapp.net') : p.jid;
        return {
          id,
          jid: id,
          lid,
    ...
    for (const g of groups || []) {
      out[g.jid] = {
        ...
        participants: (g.participants || []).map((p) => {
          ...
          const cachedPn = lid && conn.isLid?.get?.(lid);
          const raw = p.phoneNumber || (typeof cachedPn === 'string' && cachedPn.endsWith('@s.whatsapp.net') ? cachedPn : null) || p.jid;
          return raw ? (String(raw).endsWith('@') ? String(raw) : String(raw).replace(/[^0-9]/g, '') + '@s.whatsapp.net') : p.jid;
        })
      };
    }
    return out;
  ...
  conn.groupParticipantsUpdate = async (jid, participants, action) => {
    ...
    switch (action) {
      case 'add':
        return conn._client.group.addParticipants(jid, parts);
      case 'remove':
        return conn._client.group.removeParticipants(jid, parts);
      case 'promote':
        return conn._client.group.promoteParticipants(jid, parts);
      case 'demote':
        return conn._client.group.demoteParticipants(jid, parts);
      default:
  ...
  // Cache username WhatsApp per jid. Hasil negatif juga di-cache supaya jid
  // tanpa username tidak memicu request usync di setiap pesan.
  conn._usernames = conn._usernames || new Map();
  conn.Example = async (prefix, command, text, description) => {
    let response = `🚩 *Example*: ${prefix}${command} ${text}`;
    if (description) {
      response += `\n*Note*: ${description}`;
    }
    return response;
  ...
  conn.setCachedUsername = (jid, username) => {
    const key = conn.decodeJid(jid);
    if (key) conn._usernames.set(key, username || null);
  };

  /** Baca cache saja, sinkron. Untuk jalur yang tidak boleh menunggu jaringan. */
  conn.getCachedUsername = (jid) => {
    const key = conn.decodeJid(jid);
    return (key && conn._usernames.get(key)) || null;
  };

  /**
   * Username WhatsApp untuk sebuah jid, atau null kalau tidak punya.
   * @param {string} jid
   * @param {boolean} cacheOnly true = jangan pernah menyentuh jaringan
   */
  conn.getUsername = async (jid = '', cacheOnly = false) => {
    const key = conn.decodeJid(jid);
    if (!key || key.endsWith('@g.us') || key.endsWith('@broadcast') || key.endsWith('@newsletter')) return null;
    if (conn._usernames.has(key)) return conn._usernames.get(key);
    if (cacheOnly) return null;
    try {
      const results = await conn._client.profile.getUsernames([key]);
      const username = results?.find((r) => conn.decodeJid(r.jid) === key)?.username || null;
      conn._usernames.set(key, username);
      return username;
    } catch {
      // Jangan cache kegagalan sebagai "tidak punya username" — request
      // berikutnya boleh mencoba lagi.
      return null;
    }
  };
  ...
  // Bentuk Baileys: satu fungsi untuk semua app-state mutation per-chat.
  // Sebelumnya hanya delete dan clear yang diterjemahkan, jadi pemanggil yang
  // mengirim { archive }, { pin }, { mute }, atau { markRead } tidak error —
  // tapi juga tidak melakukan apa pun. Nilai yang tidak dikenal dilempar
  // supaya kegagalan terlihat, bukan diam.
  //
  // mute: milidetik durasi (Baileys) atau null untuk unmute.
  conn.chatModify = async (mods, jid) => {
    const chat = conn._client.chat;
    if (!mods || typeof mods !== 'object') throw new Error('chatModify: mods harus objek');

    if ('archive' in mods) return chat.setChatArchive(jid, !!mods.archive);
    if ('pin' in mods) return chat.setChatPin(jid, !!mods.pin);
    if ('mute' in mods) return mods.mute == null ? chat.setChatMute(jid, false) : chat.setChatMute(jid, true, Date.now() + Number(mods.mute));
    if ('markRead' in mods) return chat.setChatRead(jid, !!mods.markRead);
    if (mods.star) return chat.setMessageStar({ chatJid: jid, id: mods.star.messages?.[0]?.key?.id, fromMe: !!mods.star.messages?.[0]?.key?.fromMe, participantJid: mods.star.messages?.[0]?.key?.participant }, !!mods.star.star);
    if (mods.deleteForMe) return chat.deleteMessageForMe({ chatJid: jid, id: mods.deleteForMe.key?.id, fromMe: !!mods.deleteForMe.key?.fromMe }, { deleteMedia: !!mods.deleteForMe.deleteMedia });
    if (mods.delete) return chat.deleteChat(jid);
    if (mods.clear) return chat.clearChat(jid);

    throw new Error(`chatModify: mods tidak dikenal (${Object.keys(mods).join(', ')})`);
  };
  ...
  conn.sendMessage = async (jid, content, options = {}) => {
    ...
    let mentionJids = [...new Set([...(Array.isArray(content?.mentions) ? content.mentions : []), ...(content?.contextInfo?.mentionedJid || []), ...(Array.isArray(opts.mentions) ? opts.mentions : []), ...(opts.contextInfo?.mentionedJid || [])].filter(Boolean).map((j) => String(j)))];
    ...
    // Link preview: satu-satunya cara mengirim teks dengan thumbnail tanpa
    // mengunggah gambar. Diteruskan apa adanya ke zapo — `true` = ambil sendiri
    // dari URL-nya, objek = pakai judul/deskripsi/thumbnail yang diberikan,
    // `false` = matikan. `undefined` sengaja tidak diteruskan supaya default
    // global zapo yang berlaku.
    const previewOf = (c) => (c?.linkPreview !== undefined ? { linkPreview: c.linkPreview } : {});
    ...
    if (typeof content === 'string') {
      ...
      zapContent = { type: 'text', text: normText(String(content.text)), ...mentionCtx, ...previewOf(content) };
      ...
      const text = typeof fcontent === 'string' ? fcontent : fcontent.text || fcontent.caption || fmsg.conversation || '';
  ...
  conn.getJid = (sender) => {
    ...
    for (const chat of Object.values(conn.chats || {})) {
      ...
      const participant = chat.metadata.participants.find((p) => p.lid === jid || p.id === jid || p.jid === jid || (p.phoneNumber && jid === conn.decodeJid(p.phoneNumber)));
  ...
  conn.logger = createLogger(conn.logger);
...
`.trim();
  ...
  conn.getName = async (jid = '', withoutContact = false) => {
    ...
    // Username jauh lebih enak dibaca daripada digit LID mentah, yang tidak
    // memberi petunjuk apa pun tentang siapa pengirimnya.
    const username = await conn.getUsername(jid);
  ...
  conn.processMessageStubType = async (m) => {
    ...
    if ([WAMessageStubType.GROUP_PARTICIPANT_ADD, WAMessageStubType.GROUP_PARTICIPANT_REMOVE, WAMessageStubType.GROUP_PARTICIPANT_PROMOTE, WAMessageStubType.GROUP_PARTICIPANT_DEMOTE, WAMessageStubType.GROUP_PARTICIPANT_INVITE, WAMessageStubType.GROUP_PARTICIPANT_LEAVE, WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER].includes(m.messageStubType)) {
      try {
        ...
        // Stub parameter sering teks biasa, bukan JSON — bukan error.
      }
    }
  ...
  conn.getBuffer = async (url, options = {}) => {
    try {
      // Dulu memakai axios yang tidak pernah diimpor di file ini, jadi setiap
      // pemanggilan langsung ReferenceError. node-fetch sudah tersedia.
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.key) {
    ...
    let senderJid = m.key.fromMe ? conn.user.id : m.key.participantPn || (m.key.remoteJid?.endsWith('@lid') ? m.key.remoteJidPn : undefined) || m.key.senderPn || conn.getJid(m.key.participant || m.key.remoteJid || m.key.senderLid || m.chat || '') || m.key.participant || m.key.senderLid || m.key.remoteJid || m.participant || m.chat || '';

    m.sender = conn.decodeJid(senderJid);
    m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, conn.user.id);

    // Pengarang pesan yang SELALU terisi. key.participant hanya ada di grup dan
    // broadcast; di chat pribadi pengarangnya adalah pengirim itu sendiri.
    m.participant = m.key.participant || m.key.participantPn || m.sender;

    // Username yang ikut di key pesan: gratis, jadi sekalian isi cache supaya
    // getName() tidak perlu request usync untuk pengirim ini.
    m.senderUsername = m.key.senderUsername || conn.getCachedUsername?.(m.sender) || null;
    if (m.senderUsername && conn.setCachedUsername) conn.setCachedUsername(m.sender, m.senderUsername);
  ...
  if (m.message) {
    ...
    if (typeof m.text !== 'string') {
      if (['protocolMessage', 'messageContextInfo', 'stickerMessage', 'lottieStickerMessage', 'audioMessage', 'senderKeyDistributionMessage'].includes(m.mtype)) m.text = '';
    ...
    if (m.quoted) {
      ...
      if (m.quoted) {
        ...
        if (!m.quoted.name)
          conn
            .getName(m.quoted.sender)
            .then((n) => {
              if (n && !m.quoted.name) m.quoted.name = n;
            })
            .catch(() => {});
...
if (process.argv[1] === import.meta.filename) {
  ...
  // conn palsu: attach() cuma menempelkan method, jadi tidak butuh socket asli.
  const makeConn = (getUsernames) => {
    const conn = { ev: new EventEmitter(), _client: { profile: { getUsernames } } };
    exports.attach(conn);
    return conn;
  };
  const known = '6285891001164@s.whatsapp.net';
  const unknown = '628999000111@s.whatsapp.net';

  // Hasil positif di-cache: dua lookup, satu request
  let calls = 0;
  let conn = makeConn(async (jids) => {
    calls++;
    return jids.map((jid) => ({ jid, username: jid === known ? 'sariawan99' : null }));
  });
  assert.strictEqual(await conn.getUsername(known), 'sariawan99');
  assert.strictEqual(await conn.getUsername(known), 'sariawan99');
  assert.strictEqual(calls, 1, 'lookup kedua harus dari cache');

  // Hasil negatif juga di-cache, supaya jid tanpa username tidak memicu
  // request usync di setiap pesan yang masuk
  assert.strictEqual(await conn.getUsername(unknown), null);
  assert.strictEqual(await conn.getUsername(unknown), null);
  assert.strictEqual(calls, 2, 'hasil negatif harus di-cache');

  // cacheOnly tidak pernah menyentuh jaringan
  assert.strictEqual(await conn.getUsername('628777000111@s.whatsapp.net', true), null);
  assert.strictEqual(calls, 2, 'cacheOnly tidak boleh request');

  // Grup tidak punya username, jadi jangan sampai ditanyakan
  assert.strictEqual(await conn.getUsername('123456789@g.us'), null);
  assert.strictEqual(calls, 2, 'jid grup tidak boleh request');

  // Error jaringan mengembalikan null tapi TIDAK di-cache — request berikutnya
  // masih boleh mencoba lagi
  let failCalls = 0;
  const failing = makeConn(async () => {
    failCalls++;
    throw new Error('usync gagal');
  });
  assert.strictEqual(await failing.getUsername(known), null);
  assert.strictEqual(await failing.getUsername(known), null);
  assert.strictEqual(failCalls, 2, 'kegagalan tidak boleh di-cache sebagai "tidak punya username"');

  // getCachedUsername sinkron: dipakai smsg(), yang tidak boleh menunggu jaringan
  const sync = makeConn(async () => {
    throw new Error('getCachedUsername tidak boleh request');
  });
  assert.strictEqual(sync.getCachedUsername(known), null, 'cache kosong = null, bukan undefined');
  sync.setCachedUsername(known, 'handlenya');
  assert.strictEqual(sync.getCachedUsername(known), 'handlenya');

  // setCachedUsername mengisi cache tanpa jaringan (dipakai smsg + mex_notification)
  let seedCalls = 0;
  const seeded = makeConn(async () => {
    seedCalls++;
    return [];
  });
  seeded.setCachedUsername(known, 'darisenderkey');
  assert.strictEqual(await seeded.getUsername(known), 'darisenderkey');
  assert.strictEqual(seedCalls, 0, 'username dari key pesan tidak boleh memicu request');

  // getName memakai username sebagai fallback sebelum digit LID mentah
  const lid = '224739230494840@lid';
  const byUsername = makeConn(async (jids) => jids.map((jid) => ({ jid, username: 'handlenya' })));
  assert.strictEqual(await byUsername.getName(lid), '@handlenya', 'LID tanpa kontak harus jatuh ke username');

  // Tanpa username, perilaku lama tetap: digit LID apa adanya
  const byDigits = makeConn(async (jids) => jids.map((jid) => ({ jid, username: null })));
  assert.strictEqual(await byDigits.getName(lid), '224739230494840');

  // Nama kontak tetap menang atas username
  const byContact = makeConn(async (jids) => jids.map((jid) => ({ jid, username: 'handlenya' })));
  byContact.chats[byContact.decodeJid(known)] = { id: known, name: 'Sariawan' };
  assert.strictEqual(await byContact.getName(known), 'Sariawan', 'nama kontak lebih diutamakan');

  // smsg(): m.participant selalu terisi, walaupun key.participant tidak ada
  const msgConn = makeConn(async (jids) => jids.map((jid) => ({ jid, username: null })));
  msgConn.user = { id: '628111000222@s.whatsapp.net', jid: '628111000222@s.whatsapp.net' };
  const serialize = (key) => exports.smsg(msgConn, { key, message: { conversation: 'hai' }, messageTimestamp: { low: 1786784833 } });

  const dm = serialize({ remoteJid: '233955710214283@lid', fromMe: false, id: 'A1', remoteJidPn: known });
  assert.strictEqual(dm.isGroup, false);
  assert.strictEqual(dm.sender, known, 'DM berbasis LID harus resolve ke nomor');
  assert.strictEqual(dm.participant, known, 'di DM pengarangnya adalah pengirim');
  assert.strictEqual(dm.key.participant, undefined, 'key.participant tetap kosong di DM');

  const inGroup = serialize({ remoteJid: '123456789@g.us', fromMe: false, id: 'A2', participant: known });
  assert.strictEqual(inGroup.isGroup, true);
  assert.strictEqual(inGroup.participant, known, 'di grup pengarangnya dari key.participant');

  // Username dari key dipakai apa adanya dan ikut mengisi cache
  const withHandle = serialize({ remoteJid: known, fromMe: false, id: 'A3', senderUsername: 'sariawan99' });
  assert.strictEqual(withHandle.senderUsername, 'sariawan99');
  assert.strictEqual(msgConn.getCachedUsername(known), 'sariawan99', 'username dari key harus masuk cache');

  // Pesan berikutnya dari pengirim yang sama ambil dari cache, tanpa key
  const fromCache = serialize({ remoteJid: known, fromMe: false, id: 'A4' });
  assert.strictEqual(fromCache.senderUsername, 'sariawan99', 'smsg baca cache saat key tidak membawa username');

  // chatModify(): terjemahan bentuk Baileys ke client.chat. Yang penting di
  // sini bukan cuma pemetaannya, tapi juga bahwa mods tak dikenal melempar —
  // dulu ia diam saja dan pemanggil menyangka pengaturannya tersimpan.
  const chatCalls = [];
  const chatConn = makeConn(async (jids) => jids.map((jid) => ({ jid, username: null })));
  chatConn._client.chat = new Proxy(
    {},
    {
      get:
        (_, method) =>
        (...args) =>
          chatCalls.push([method, ...args])
    }
  );
  const modify = async (mods) => {
    chatCalls.length = 0;
    await chatConn.chatModify(mods, known);
    return chatCalls[0];
  };

  assert.deepStrictEqual(await modify({ archive: true }), ['setChatArchive', known, true]);
  assert.deepStrictEqual(await modify({ archive: false }), ['setChatArchive', known, false], 'archive: false harus unarchive, bukan diabaikan');
  assert.deepStrictEqual(await modify({ pin: true }), ['setChatPin', known, true]);
  assert.deepStrictEqual(await modify({ markRead: true }), ['setChatRead', known, true]);
  assert.deepStrictEqual(await modify({ mute: null }), ['setChatMute', known, false], 'mute null = unmute');
  assert.deepStrictEqual((await modify({ delete: true, lastMessages: [] })).slice(0, 2), ['deleteChat', known], 'bentuk lama clearchat.js harus tetap jalan');
  assert.deepStrictEqual((await modify({ clear: 'all' })).slice(0, 2), ['clearChat', known]);

  // mute durasi: milidetik dari sekarang, jadi hasilnya harus di masa depan
  const [muteMethod, , muteOn, muteUntil] = await modify({ mute: 8 * 3600 * 1000 });
  assert.strictEqual(muteMethod, 'setChatMute');
  assert.strictEqual(muteOn, true);
  assert.ok(muteUntil > Date.now(), `mute harus berakhir di masa depan, dapat ${muteUntil}`);

  await assert.rejects(() => chatConn.chatModify({ tidakAda: true }, known), /tidak dikenal/, 'mods asing harus melempar, bukan diam');
  await assert.rejects(() => chatConn.chatModify(null, known), /harus objek/);

  // linkPreview: teks + thumbnail tanpa mengunggah gambar. Dulu field ini
  // dibuang diam-diam di sini, jadi plugin mengira preview-nya terkirim.
  const sendConn = makeConn(async (jids) => jids.map((jid) => ({ jid, username: null })));
  const sent = [];
  sendConn._client.message = { send: async (jid, c, o) => (sent.push({ jid, content: c, options: o }), { id: 'X1' }) };
  const sendText = async (content) => {
    sent.length = 0;
    await sendConn.sendMessage(known, content);
    return sent[0].content;
  };

  const preview = { title: 'Menu', description: 'daftar perintah', thumbnail: { bytes: new Uint8Array([1, 2, 3]) } };
  assert.deepStrictEqual((await sendText({ text: 'https://contoh.dev', linkPreview: preview })).linkPreview, preview, 'objek preview harus diteruskan utuh');
  assert.strictEqual((await sendText({ text: 'https://contoh.dev', linkPreview: true })).linkPreview, true, 'true = zapo ambil sendiri');
  assert.strictEqual((await sendText({ text: 'https://contoh.dev', linkPreview: false })).linkPreview, false, 'false harus ikut terkirim, bukan dianggap kosong');
  assert.strictEqual('linkPreview' in (await sendText({ text: 'halo' })), false, 'tanpa linkPreview, default global zapo yang berlaku');

  console.log('simple.js self-check OK');
  process.exit(0);
}
