...
import { displayJidNumber as displayNumber } from '../simple.js';
...
export default async function printMessage(m, conn = { user: {} }) {
  ...
  // Satu baris kosong sebelum tiap entri: tanpa ini pesan yang datang
  // berurutan menyatu jadi blok yang sulit dipisah mata.
  console.log();
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (body.length < 4096)
      body = body.replace(urlRegex, (url, i, text) => {
        let end = url.length + i;
        return i === 0 || end === text.length || (/^\s$/.test(text[end]) && /^\s$/.test(text[i - 1])) ? chalk.blueBright(url) : url;
      });
    body = body.replace(mdRegex, mdFormat(4));
    if (m.mentionedJid)
      for (let user of m.mentionedJid) {
        let userJid = await resolveJid(conn, user, m.isGroup ? m.chat : '');
        body = body.replace('@' + userJid.split`@`[0], chalk.blueBright('@' + (await conn.getName(userJid))));
      }
    console.log(indent(m.error != null ? chalk.red(body) : m.isCommand ? chalk.yellow(body) : body));
  }
...
if (process.argv[1] === import.meta.filename) {
  ...
  // Setiap entri diawali satu baris kosong sebagai pemisah. Diuji sekali di
  // sini, lalu dibuang di `entry()` supaya assertion lain tetap membaca
  // baris pertama yang berarti.
  const raw = await capture({ mtype: 'conversation', msg: { text: 'x' }, text: 'x' });
  assert.strictEqual(raw[0], '', 'entri harus diawali baris kosong sebagai pemisah');
  const entry = async (m) => (await capture(m)).slice(1);
  ...
  const plain = await entry({ mtype: 'conversation', msg: { text: 'halo bot' }, text: 'halo bot' });
  ...
  const own = await entry({ mtype: 'conversation', msg: { text: 'ok' }, text: 'ok', fromMe: true });
  ...
  const handle = await entry({ mtype: 'conversation', msg: { text: 'hai' }, text: 'hai', senderUsername: 'sariawan99' });
  assert.ok(handle[0].includes('@sariawan99'), handle[0]);
  const noHandle = await entry({ mtype: 'conversation', msg: { text: 'hai' }, text: 'hai' });
  assert.ok(!noHandle[0].includes('@'), noHandle[0]);

  // Durasi audio ikut di baris meta, bukan baris sendiri
  const ptt = await entry({ mtype: 'audioMessage', msg: { fileLength: { low: 120500 }, ptt: true, seconds: 8 } });
  ...
  const stub = await entry({ mtype: 'conversation', msg: { text: '' }, text: '', chat: '123@g.us', isGroup: true, messageStubType: 21, messageStubParameters: ['Grup Baru'] });
  assert.ok(
