...
class BotcahxApi {
  get apikey() {
    return global.config.api.botcahx.key;
  }

  url(path, params = {}) {
    const url = new URL(path || '/', global.config.api.botcahx.url);
...
if (process.argv[1] === import.meta.filename) {
  ...
  global.config = { api: { botcahx: { url: 'https://api.botcahx.eu.org', key: 'TESTKEY' } } };
