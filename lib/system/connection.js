...
function makeWASocketBase(connectionOptions = {}) {
  ...
  client.on('mex_notification', (ev) => {
    ...
    if (ev.kind === 'message_capping' && ev.cappingStatus && ev.cappingStatus !== 'NONE') {
      ...
      console.log(chalk.bgRed.white(` KUOTA PESAN: ${ev.cappingStatus}${quota} `));
    }
  });
...
export async function createClient(authFile) {
  ...
  if (process.argv.includes('--reset-session')) {
    console.log(chalk.yellow(`-- --reset-session: menghapus ${storeSqlitePath} --`));
...
async function askPhoneNumber(question) {
  ...
  if (phoneNumber.length >= 10) {
    console.log(chalk.blueBright(`Pakai nomor bot dari config.js: ${phoneNumber}`));
    return phoneNumber;
  }
  do {
    phoneNumber = (await question(chalk.blueBright('Masukkan nomor bot diawali kode negara. Contoh: 628xxx\n'))).replace(/\D/g, '');
    if (phoneNumber.length < 10) console.log(chalk.red('Nomor tidak valid, coba lagi.'));
...
export async function authenticate(conn, { rl, question }) {
  ...
  if (!usePair) {
    console.log(chalk.blueBright('Mode QR aktif. QR hanya muncul kalau sesi belum tersimpan.'));
    closePrompt();
    return conn;
  }

  console.log(chalk.blueBright('Mode Pairing Code aktif. Kode hanya diminta kalau sesi belum tersimpan.'));
  conn._client.once('auth_pairing_required', async () => {
    try {
      ...
      console.error(chalk.red(`Gagal meminta pairing code: ${e?.message || e}`));
    }
  });
