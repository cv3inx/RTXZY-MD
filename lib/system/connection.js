...
import simple, { generateMessageID, displayJidNumber } from '../simple.js';
...
function makeWASocketBase(connectionOptions = {}) {
  ...
  client.on('connection', (ev) => {
    ...
    if (ev.status === 'open') {
      ...
      const me = conn.user;
      const nomor = me?.jid ? displayJidNumber(me.jid) : '';
      log.ready(`Tersambung  ${[me?.name, nomor].filter(Boolean).join('  ·  ') || 'siap'}`);
      if (ev.isNewLogin) log.info('Perangkat baru ditautkan — restart berikutnya tidak perlu scan lagi');
...
export async function connectionUpdate(update) {
  ...
  if (qr && global.useQR) {
    log.info('Sesi belum ada — scan QR di bawah dari WhatsApp › Perangkat tertaut');
    console.log(await QRCode.toString(qr, { type: 'terminal', small: true }));
    log.detail('QR rusak atau terlalu besar? Buka link ini:');
    log.detail(`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(qr)}`);
  }
