...
import { WaClient, createStore, createNoopLogger, fetchLatestWaWebVersion } from 'zapo-js';
...
async function resolveWaVersion() {
  try {
    const { parts } = await fetchLatestWaWebVersion();
    return parts;
  } catch {
    return [2, 3000, 10179];
  }
...
export async function authenticate(conn, { rl, question, connectionOptions, storeSqlitePath }) {
  ...
  if (!global.isInit && sqliteSize === 0 && !conn.authState.creds.registered && !conn.authState.creds.me) {
    ...
    global.isInit = true;
  } else if (!global.isInit && sqliteSize > 0 && !conn.authState.creds.registered && !conn.authState.creds.me) {
