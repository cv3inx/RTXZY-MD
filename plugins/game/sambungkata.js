import { sKata, cKata } from '.././lib/games/sambung-kata.js';
  ...
  if (id in conn.skata) {
    ...
    if (room.status == 'play') {
      if (!room.waktu._destroyed && !room.diam)
        return conn.reply(m.chat, `Hii @${m.sender.split`@`[0]}, Masih ada game berlangsung di chat ini\nTunggu hingga game berakhir\nLalu ikut bergabung`, room.chat, { contextInfo: { mentionedJid: member } }).catch((e) => {
          return !1;
        }); // ketika naileys err
    ...
    if (text == 'start' && room.status == 'wait') {
      ...
      room.waktu_list = setTimeout(() => {
        conn.reply(m.chat, `Sambung kata tidak dimulai (Cancel)`, room.chat).then(() => {
          delete conn.skata[id];
        });
      }, 120000);
