...
const handler = {
  ...
  run: async (m, { conn, command, usedPrefix, args }) => {
    ...
    if (value === 'create') {
      ...
      if (ww[chat].owner !== sender) return m.reply(`Hanya pemain yang membuat room yang dapat memulai permainan`);
      let list1 = '';
      let list2 = '';
      let player = [];
      ...
      for (let i = 0; i < ww[chat].player.length; i++) {
        ...
        if (ww[chat].player[i].role === 'werewolf') {
          if (ww[chat].player[i].isdead != true) {
            var text = `Hai ${await conn.getName(ww[chat].player[i].id)}, Kamu telah dipilih untuk memerankan *Werewolf* ${emoji_role('werewolf')} pada permainan kali ini, silahkan pilih salah satu player yang ingin kamu makan pada malam hari ini\n*LIST PLAYER*:\n${list2}\n\nKetik *.wwpc kill nomor* untuk membunuh player`;
          ...
          if (ww[chat].player[i].isdead != true) {
            let text = `*⌂ W E R E W O L F - G A M E*\n\nHai ${await conn.getName(ww[chat].player[i].id)} Peran kamu adalah *Warga Desa* ${emoji_role('warga')}, tetap waspada, mungkin *Werewolf* akan memakanmu malam ini, silakan masuk kerumah masing masing.\n*LIST PLAYER*:\n${list1}`;
            await conn.sendMessage(ww[chat].player[i].id, {
              text: text,
              mentions: player
            });
          ...
          if (ww[chat].player[i].isdead != true) {
            let text = `Hai ${await conn.getName(ww[chat].player[i].id)} Kamu telah terpilih  untuk menjadi *Penerawang* ${emoji_role('seer')}. Dengan sihir yang kamu punya, kamu bisa mengetahui peran pemain pilihanmu.\n*LIST PLAYER*:\n${list1}\n\nKetik *.wwpc dreamy nomor* untuk melihat role player`;
          ...
          if (ww[chat].player[i].isdead != true) {
            let text = `Hai ${await conn.getName(ww[chat].player[i].id)} Kamu terpilih untuk memerankan *Malaikat Pelindung* ${emoji_role('guardian')}, dengan kekuatan yang kamu miliki, kamu bisa melindungi para warga, silahkan pilih salah 1 player yang ingin kamu lindungi\n*LIST PLAYER*:\n${list1}\n\nKetik *.wwpc deff nomor* untuk melindungi player`;
          ...
          if (ww[chat].player[i].isdead != true) {
            let text = `Hai ${await conn.getName(ww[chat].player[i].id)} Kamu terpilih sebagai Penyihir ${emoji_role('sorcerer')}, dengan kekuasaan yang kamu punya, kamu bisa membuka identitas para player, silakan pilih 1 orang yang ingin kamu buka identitasnya\n*LIST PLAYER*:\n${list2}\n\nKetik *.wwpc sorcerer nomor* untuk melihat role player`;
      ...
      if (b === false) return m.reply('Player tidak terdaftar!');
