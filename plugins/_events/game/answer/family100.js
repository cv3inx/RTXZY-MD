...
export default {
  async before(m) {
    ...
    if (!text) return !0;
    if (text === 'nyerah') {
      let allAnswers = room.jawaban.map((j, i) => `(${i + 1}) ${j}`).join('\n');
    ...
    if (index < 0) {
      ...
      if (belumTerjawab.length > 0) {
        let maxSim = Math.max(...belumTerjawab.map((j) => similarity(j, text)));
    ...
    if (!global.db.data.users[m.sender]) global.db.data.users[m.sender] = { money: 0 };
...
${room.jawaban
  .map((j, i) => {
    return room.terjawab[i] ? `(${i + 1}) ${j} @${room.terjawab[i].split('@')[0]}` : '';
  })
  .filter(Boolean)
  .join('\n')}
