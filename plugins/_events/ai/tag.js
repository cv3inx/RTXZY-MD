...
let handler = async (m, { conn, text, command, Api }) => {
  // kosong
};

handler.before = async (m, { conn, Api }) => {
  try {
    if (!m.isGroup) return;
    conn.selfai = conn.selfai || {};
    if (m.isZapo && m.fromMe) return;
    if (m.mentionedJid && m.mentionedJid.length > 0) {
      const botNumbers = new Set([conn.user?.jid, conn.user?.id, conn._client?.getCredentials?.()?.meLid].filter(Boolean).map((j) => String(j).split('@')[0].split(':')[0]));

      const isMention = m.mentionedJid.some((mentioned) => botNumbers.has(String(mentioned).split('@')[0].split(':')[0]));
      ...
      if (isMention) {
        ...
        if (filter.toLowerCase().startsWith('/imagine')) {
          ...
          try {
            ...
            const response = await Api.get('/api/search/openai-image', { text: imagePrompt });
        ...
        if (!filter) {
          const empty_response = [`Hi ${m.name}, how can I assist you today?`, `Ada yang bisa saya bantu, ${m.name}?`, `Hai ${m.name}, silakan beritahu saya apa yang Anda butuhkan.`, `${m.name}, saya siap membantu. Ada pertanyaan?`, `Apa yang ingin kamu diskusikan, ${m.name}?`];
        ...
        /**
                ...
                **/
        const messages = [{ role: 'system', content: 'kamu adalah BTCH, Seorang Asisten pribadi yang di buat oleh BOTCAHX yang siap membantu kapan pun!' }, { role: 'assistant', content: `Saya BTCH, asisten pribadi yang siap membantu kamu kapan pun! Apa yang bisa saya bantu hari ini?` }, ...previousMessages.map((msg, i) => ({ role: i % 2 === 0 ? 'user' : 'assistant', content: msg })), { role: 'user', content: filter }];
        ...
        try {
          ...
          if (res && res.result) {
            ...
            conn.selfai[m.sender].sessionChat = [...conn.selfai[m.sender].sessionChat, filter, res.result];
          } else {
