...
export default {
  async all(m, chatUpdate) {
    ...
    const interactiveMsg = m.message.buttonsResponseMessage || m.message.templateButtonReplyMessage || m.message.listResponseMessage;

    if (!interactiveMsg) return;

    const id = interactiveMsg.selectedButtonId || interactiveMsg.selectedId || interactiveMsg.singleSelectReply?.selectedRowId;

    const text = interactiveMsg.selectedDisplayText || interactiveMsg.description || '';
