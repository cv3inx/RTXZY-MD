...
const handler = {
  command: ['diffusion', 'stablediffusion', 'diff'],
  help: ['diffusion', 'stablediffusion', 'diff'],
  tags: ['ai'],
  limit: true,
  private: false,
  group: false,
  run: async (m, { text, conn, usedPrefix, command, Api }) => {
    ...
    const response = await Api.get('/api/search/stablediffusion', { text: prompt });
