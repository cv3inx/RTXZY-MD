const handler = {
  command: ['ffstalk', 'mlstalk', 'mlstalk2', 'supersusstalk', 'npmstalk', 'repostalk', 'genshinstalk', 'stalkgenshin', 'hokstalk', 'robloxstalk'],
  help: ['ffstalk', 'mlstalk', 'mlstalk2', 'supersusstalk', 'npmstalk', 'repostalk', 'genshinstalk', 'stalkgenshin', 'hokstalk', 'robloxstalk'],
  tags: ['stalk'],
  limit: true,
  run: async (m, { conn, args, text, usedPrefix, command, Api }) => {
    if (command == 'ffstalk') {
      ...
      m.reply(wait);
      try {
        let ff = await Api.get('/api/stalk/ff', { id: text }).then((res) => res.json());
    ...
    if (command == 'mlstalk') {
      ...
      try {
        let ml = await Api.get('/api/stalk/ml', { id, server }).then((res) => res.json());
    ...
    if (command == 'mlstalk2') {
      ...
      let [id, server] = text.split('|');
      try {
        let ml = await Api.get('/api/stalk/ml-v2', { id, server }).then((res) => res.json());
    ...
    if (command == 'supersusstalk') {
      ...
      m.reply(wait);
      try {
        let sus = await Api.get('/api/stalk/supersus', { id: text }).then((res) => res.json());
    ...
    if (command == 'npmstalk') {
      ...
      m.reply(wait);
      try {
        let npm = await Api.get('/api/stalk/npm', { name: text }).then((res) => res.json());
    ...
    if (command == 'repostalk') {
      ...
      m.reply(wait);
      try {
        let repo = await Api.get('/api/stalk/repo', { repo: text }).then((res) => res.json());
    ...
    if (command == 'genshinstalk' || command == 'stalkgenshin') {
      ...
      m.reply(wait);
      try {
        let genshin = await Api.get('/api/stalk/genshin', { id: text }).then((res) => res.json());
    ...
    if (command == 'hokstalk') {
      ...
      m.reply(wait);
      try {
        let { result } = await Api.get('/api/stalk/hok', { id: text }).then((res) => res.json());
    ...
    if (command == 'robloxstalk') {
      ...
      m.reply(wait);
      try {
        let roblox = await Api.get('/api/stalk/roblox', { username: text }).then((res) => res.json());
