let handler = async (m, { conn, usedPrefix, text, command, Api }) => {
  if (!text) throw `Example: \`${usedPrefix + command} kadita\``;

  try {
    m.reply(wait);
    const res = await (await Api.get('/api/search/heroml', { hero: text })).json();
    ...
    content += ['Previous', 'Current', 'Next'].map((key) => `  ◦ ${key}: ${heroNumber[key.toLowerCase()] || 'N/A'}`).join('\n') + '\n\n';
    content += `*📊 Ratings:*\n`;
    const ratings = hero.ratings || {};
    content += ['Durability', 'Offense', 'Control Effect', 'Difficulty'].map((key) => `  ◦ ${key}: ${ratings[key.toLowerCase()] || 0}/10`).join('\n') + '\n\n';
    ...
    content += Array.isArray(hero.relationships) && hero.relationships.length > 0 ? hero.relationships.map((r) => `  ◦ ${r}`).join('\n') : '  ◦ None\n';
    content += '\n';
    ...
    content +=
      statFields
        .map(([label, key, valueType = 'level']) => {
          const stat = stats[key] || {};
          return valueType === 'level' ? `  ◦ ${label}: ${stat.level1 || 'N/A'} => ${stat.level15 || 'N/A'}` : `  ◦ ${label}: ${stat.value || 'N/A'}`;
        })
        .join('\n') + '\n\n';

    content += `*🪄 Abilities:*\n`;
    content += Array.isArray(hero.abilities) && hero.abilities.length > 0 ? hero.abilities.map((item) => `  ◦ ${item.name || 'Unknown'}: ${item.description || 'No description available'}`).join('\n') : '  ◦ No abilities available\n';
    content += '\n\n';
