const handler = async (m, { conn, command, text, Api }) => {
  if (!text) return conn.reply(m.chat, 'Masukkan teks untuk dicari!', m);

  try {
    await m.reply(wait);
    const response = await Api.get('/api/search/google', { text1: text });
    const data = await response.json();

    if (!data.status || !data.result?.length) throw new Error('Pencarian gagal atau tidak ada hasil');

    const msg = data.result.map(({ title, url, snippet }, index) => `${index + 1}. *${title}*\n🌐 ${url}\n📝 ${snippet || 'Tidak ada deskripsi tersedia'}`).join('\n\n');
