let handler = async (m, { usedPrefix, command, text, Api }) => {
  try {
    ...
    if (text.toLowerCase() === 'stock') {
      let res = await (await Api.get('/api/webzone/grow-and-garden-stock')).json();
      ...
      let res = await (await Api.get('/api/webzone/grow-and-garden-weather')).json();
