const listnabi = ['adam', 'ayyub', 'daud', 'dzulkifli', 'harun', 'hud', 'ibrahim', 'idris', 'ilyas', 'ilyasa', 'isa', 'ishaq', 'ismail', 'luth', 'muhammad', 'musa', 'nuh', 'sholeh', 'sulaiman', 'syuaib', 'yahya', 'yaqub', 'yunus', 'yusuf', 'zakariya'];
...
let handler = async (m, { conn, text, usedPrefix, command, Api }) => {
  ...
  try {
    await m.reply(wait);
    let res = await Api.get('/api/muslim/kisahnabi', { nabi: text });
