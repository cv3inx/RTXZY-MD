const handler = {
  command: ['checkapi', 'api', 'cekakseskey', 'akseskey'],
  help: ['checkapi', 'api', 'cekakseskey', 'akseskey'],
  tags: ['main'],
  owner: true,
  run: async (m, { conn, command, Api }) => {
    try {
      await m.reply(wait);

      if (command == 'checkapi' || command == 'api') {
        let api = await Api.get('/api/checkkey');
        let body = await api.json();
        let { limit, premium, expired, todayHit, totalHit } = body.result;

        let capt = `乂 *C H E C K   A P I K E Y*\n\n`;
        capt += `◦ *Limit*: ${limit}\n`;
        capt += `◦ *Premium*: ${premium}\n`;
        capt += `◦ *Expired*: ${expired}\n`;
        capt += `◦ *Today Hit*: ${todayHit}\n`;
        capt += `◦ *Total Hit*: ${totalHit}\n\n`;
        await conn.reply(m.chat, capt, m);
      }

      if (command == 'cekakseskey' || command == 'akseskey') {
        let api = await Api.get('/api/cek-aksesKey', { aksesKey });
        let body = await api.json();
        let { limit, usageCount, expired, type } = body.data;

        let capt = `乂 *C H E C K   A K S E S K E Y*\n\n`;
        capt += `◦ *Type*: ${type}\n`;
        capt += `◦ *Limit*: ${limit}\n`;
        capt += `◦ *Usage Count*: ${usageCount}\n`;
        capt += `◦ *Expired*: ${expired}\n\n`;
        await conn.reply(m.chat, capt, m);
      }
    } catch (e) {
      throw eror;
    }
  }
};

export default handler;
