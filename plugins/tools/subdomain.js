...
const handler = {
  command: ['createsub', 'checksub', 'createdomain', 'createsubdomain'],
  help: ['createsub', 'checksub', 'createdomain', 'createsubdomain'],
  tags: ['tools'],
  premium: false,
  limit: true,
  run: async (m, { text, usedPrefix, command, Api }) => {
    if (command === 'checksub') {
      ...
      try {
        m.reply(wait);
        const checkData = await whois(subdomain, domain, Api);
    ...
    if (command === 'createsub' || command === 'createdomain' || command === 'createsubdomain') {
      ...
      try {
        ...
        let response = await create(subdomain, domain, type, content, isProxied, Api);
...
async function whois(subdomain, domain, Api) {
  const url = Api.url('/api/tools/whois-subdo', { subdomain, domain });
  try {
...
async function create(subdomain, domain, type, content, proxied, Api) {
  const url = Api.url('/api/tools/create-subdo', { subdomain, domain, type, content, proxied });
  try {
    const response = await fetch(url, { method: 'GET' });
    return await response.json();
  } catch (error) {
