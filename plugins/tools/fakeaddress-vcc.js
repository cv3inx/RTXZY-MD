const handler = {
  command: ['vccgenerator', 'vccgen', 'fakeaddress', 'addressgenerator'],
  help: ['vccgenerator', 'vccgen', 'fakeaddress', 'addressgenerator'],
  tags: ['tools'],
  limit: true,
  run: async (m, { conn, args, text, usedPrefix, command, Api }) => {
    if (command == 'vccgenerator' || command == 'vccgen') {
      ...
      m.reply(wait);
      try {
        let vcc = await Api.get('/api/tools/vccgen', { jumlah }).then((res) => res.json());
    ...
    if (command == 'fakeaddress' || command == 'addressgenerator') {
      ...
      try {
        if (!text) {
          let list_country = await Api.get('/api/tools/random-address', { country: '' }).then((res) => res.json());
          ...
          let address = await Api.get('/api/tools/random-address', { country: text }).then((res) => res.json());
