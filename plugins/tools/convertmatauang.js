const handler = {
  command: ['matauang', 'convertuang', 'konversiuang'],
  help: ['matauang', 'convertuang', 'konversiuang'],
  tags: ['tools'],
  limit: true,
  run: async (m, { text, usedPrefix, command, Api }) => {
    ...
    m.reply(wait);
    try {
      let res = await Api.get('/api/tools/cvuang', { from: from.toUpperCase(), to: to.toUpperCase(), jumlah });
