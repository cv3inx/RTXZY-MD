...
const handler = {
  command: ['tempmail', 'cekmail', 'checkmail'],
  tags: ['tools'],
  help: ['tempmail', 'cekmail', 'checkmail'],
  limit: true,
  run: async (m, { conn, command, usedPrefix, Api }) => {
    ...
    if (command === 'tempmail') {
      ...
      try {
        let res = await Api.get('/api/tools/create-temp-mail')
          .then((r) => r.json())
          .then((data) => ({ data }));
      ...
      try {
        let res = await Api.get('/api/tools/cek-msg-tmp-mail', { email })
          .then((r) => r.json())
          .then((data) => ({ data }));
