const handler = {
  help: ['potensikeberuntungan'],
  tags: ['fun'],
  command: /^(potensikeberuntungan)$/i,
  group: false,
  limit: true,
  run: async (m, { conn, text, usedPrefix, command, Api }) => {
    ...
    try {
      ...
      let res = await Api.get('/api/primbon/potensikeberuntungan', { nama, tanggal: tanggal1, bulan: bulan1, tahun: tahun1 });
      let json = await res.json();
      let anu = [`―-POTENSI KEBERENTUNGAN-―\n\nNama: ${json.result.message.nama}\n\nTanggal lahir: ${json.result.message.tgl_lahir}\n\nHASIL: ${json.result.message.result}`];
      if (json.status) {
